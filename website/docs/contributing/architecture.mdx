...
- **`EMIT_PER_USER_METRICS=true`** enables four `id`-labeled gauges (one series per user — unbounded). Default off. The `*_by_email_domain` variants are bounded by company domains and stay on.
