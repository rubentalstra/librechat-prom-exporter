...
| `librechat_transaction_cost_by_user` | gauge | `email` | Only emitted when `EMIT_PER_USER_METRICS=true`. |
| `librechat_transaction_token_sum_by_model_user` | gauge | `model`, `tokenType`, `email` | Same gate. |
...
| `librechat_agent_usage_by_user_count` | gauge | `agent`, `email` | Gated by `EMIT_PER_USER_METRICS=true`. |
