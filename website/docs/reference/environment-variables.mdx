...
| Variable                | Default   | Description                                                                                                                                                                                                                                                                           |
| ----------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EMIT_PER_USER_METRICS` | `false`   | When `true`, emits four high-cardinality `id`-labeled metrics — one Prometheus series per user. **Off by default** to protect large deployments. See [Cardinality](#cardinality) below.                                                                                               |
| `ANONYMIZE_EMAIL_LABEL` | `true`    | Controls the value of the `id` label on the four cardinality metrics. When `true` (default), the value is the user's Mongo `_id` (24-char ObjectId). When `false`, it's the real email address. The label name stays `id` either way so dashboards don't need to change.              |
| `METRICS_USER_ID_SALT`  | _(unset)_ | Optional secret salt. When set, the `id` label becomes a **pseudonymous 16-char HMAC-SHA256** hex value instead of the raw MongoDB id or email. This prevents anyone with `/metrics` access from cross-referencing users against the database, even if `ANONYMIZE_EMAIL_LABEL=false`. |
...
Four metrics carry an `id` label and emit one Prometheus series per user. On large deployments this grows unbounded, so they are gated behind `EMIT_PER_USER_METRICS=true`:
...
- `librechat_balance_credits_by_user{id}`
...
### Pseudonymizing user labels

When `METRICS_USER_ID_SALT` is set, each `id` label is replaced with a deterministic, 16-character HMAC-SHA256 hex value:

- The same LibreChat user always maps to the same `id` across scrapes, so Grafana graphs remain continuous.
- Without the salt, the label cannot be reversed back to the original user id or email — even with full database access.
- Use a long, random string for the salt (e.g., `openssl rand -hex 32`).
