...
| Variable                | Default   | Description                                                                                                                                                                                 |
|-------------------------|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `EMIT_PER_USER_METRICS` | `false`   | When `true`, emits three high-cardinality `email`-labeled metrics — one Prometheus series per user. **Off by default** to protect large deployments. See [Cardinality](#cardinality) below. |
...
Prometheus charges (and slows down) per **unique label combination**, not per metric. Adding a label with many possible values (like `email`) creates one time series per value — easily tens of thousands on a busy LibreChat install.
:::
